...
// Set initial "Memory" far apart so the first detection locks to one
...
// --- MAIN CAMERA & AI FUNCTION ---
async function startCamera() {
    const video = document.getElementById("cameraFeed");
    const canvas = document.getElementById("overlayCanvas");
    const ctx = canvas.getContext("2d");

    // 1. RENDER LOOP (Handles all Drawing)
    function render() {
    ...
    // 3. AI DETECTION (Update Data Only)
    pose.onResults((results) => {
        if (results.poseLandmarks) {
            ...
            // Identity logic: find closest player memory
